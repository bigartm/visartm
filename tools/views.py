    ...
    if request.method == 'POST':
        with get_temp_folder() as folder:
            ...
            response['Content-Disposition'] = (
                'attachment; filename=%s.uci.zip' % name)
            return response
    ...
    if request.method == 'POST':
        ...
        if not access_token:
            return HttpResponse(
                ...
                "Use this <a href='/accounts/vk_get_token'>link</a>.")
        ...
        with get_temp_folder() as folder:
            ...
            response['Content-Disposition'] = (
                'attachment; filename=%s.zip' % domain)
            return response
...
def get_vk_access_token(request):
    if 'vk_access_token' not in request.COOKIES:
        return None
