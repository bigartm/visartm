...
# Makes linear transformation, such as minimal non-diagonal element
# becomes 0 and maximal element becomes 1.
...
for metric in metrics.metrics_list:
    ...
    # scatter_and_regression(ax, qual.flatten_symmetric_matrix(dist),
    #     qual.flatten_symmetric_matrix(C))
    ax.scatter(qual.flatten_symmetric_matrix(
        dist), qual.flatten_symmetric_matrix(C))
    ...
    for mode in modes:
        perm = arranging_base.get_arrangement_permutation(
            dist, mode, model=research.model)
        answers.append([
        ...
        if mode == "hamilton":
            ...
            for target_metric in metrics.metrics_list:
                DDC = qual.DDC(
                    normalize_metric_matrix(
                        dist_all[target_metric]), perm)
