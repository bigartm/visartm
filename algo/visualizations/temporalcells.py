...
def visual(vis, params):
    ...
    for x in range(dates_count):
        for y in range(topics_count):
            ...
            intense = [
                size / max_size,
                size / row_max_size[y],
                size / column_max_size[x]]
            cells_send.append(
                {"X": x, "Y": y, "intense": intense, "docs": cells[x][y]})

    topics_send = [{"Y": topic.spectrum_index,
                    "topwords": top_words_html(topic.top_words(count=15)),
                    "name": ' '.join(re.findall(r"[\w']+", topic.title)[0:2])}
                   for topic in topics]
    ...
    if model.layers_count > 1:
        high_topics = Topic.objects.filter(
            model=model, layer=model.layers_count - 1)
        high_topics_temp = []
        for topic in high_topics:
            try:
                children = TopicInTopic.objects.filter(parent=topic)
                positions = [
                    relation.child.spectrum_index for relation in children]
                avg = sum(positions) / float(len(positions))
                high_topics_temp.append({
                    "topic": topic,
                    "mass_center_y": avg,
                    "name": ' '.join(re.findall(r"[\w']+", topic.title)[0:2]),
                    "positions": positions})
