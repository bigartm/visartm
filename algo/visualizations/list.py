...
def visual(vis, params):
	...
	topic_names = [truncate(topic.top_words_list(count=20)) for topic in topics]
	N = len(topic_names)
	topic_names = ["%d. %s</span>" % (i+1, topic_names[i]) for i in range(N)]
	return "<div style='font-family: Geneva, Arial, Helvetica, sans-serif;'>" + \
