...
def visual(vis, params):
	...
	topics = TopicInTopic.objects.filter(model=vis.model, parent = root_topic).order_by("child__spectrum_index")
	return "<ul>" + "".join([tree_presentation(topic.child, vis.model) for topic in topics]) + "</ul>"
