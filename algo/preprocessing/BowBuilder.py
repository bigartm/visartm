...
class BowBuilder:
    ...
    def get_word_id(self, word_text):
        ...
        if self.filtered_words_index == None:
            return word_id
        else:
    ...
    def begin_new_doc(self, doc_id):
        if not doc_id in self.word_index_dict:
    ...
    def add_word_text(self, word_text, line_num, init_pos, length):
        ...
        if not word_id in self.cur_doc:
    ...
    def filter_words(self):
        ...
        with open(os.path.join(self.uci_dir, "vocab." + self.dataset_name + ".txt"), "w", encoding = "utf-8") as f:
        ...
        for doc_id, doc in self.docs.items():
            ...
            for word_id, count in doc.items():
                if word_id in self.filtered_words_index:
                    ...
                    entries.append(str(doc_id) + " " + word_id_f + " " + str(count))
                    empty = False
            if empty:
                entries.append(str(doc_id) + " " + str(randint(0,1000)) + " 1")
                print("Warning! Empty document! (" + str(doc_id) + ")")
        
        with open(os.path.join(self.uci_dir, "docword." + self.dataset_name + ".txt"), "w") as f:
