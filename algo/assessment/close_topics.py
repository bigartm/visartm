...
def alter_problem(problem, request):
    ...
    if POST["action"] == "change_model":
        if len(AssessmentTask.objects.filter(problem=problem)) > 0:
            raise ValueError(
                "Cannot change model, because something already is assessed!")
        problem.model = ArtmModel.objects.get(id=POST["model_id"])
        topics = dict()
        for topic in Topic.objects.filter(
                model=problem.model,
                layer=problem.model.layers_count):
            topics[str(topic.index_id)] = 0
        N = len(topics)
        matrix = [[0 for j in range(N)] for i in range(N)]
...
def get_task_context(task):
    ...
    ans["topics"] = Topic.objects.filter(
        model=target_topic.model,
        layer=target_topic.layer)
    # ans["topics"] = [x["topic"] for x
    # in task.problem.model.get_related_topics(target_topic, metric="jaccard")]

    Topic.objects.filter(model=task.problem.model,
                         layer=ans["target_topic"].layer).order_by("title")
    return ans
