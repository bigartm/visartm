...
# Defines total number of assessment tasks.
# It will be ASSESS_RATIO times number of topics.
ASSESS_RATIO = 3

# How many topics will be shown in one assessment tasks.
ASSESS_LENGTH = 6
...
def initialize_problem(problem):
    if problem.model:
        ...
        problem.params = json.dumps(
            {"topics_count": N, "tasks_left": N * ASSESS_RATIO})
        problem.save()
...
def alter_task(task, POST):
    ...
    if POST["action"] == "topic_up":
        if pos == 0:
            return
        answer["topics"][pos - 1], answer["topics"][pos] = \
            answer["topics"][pos], answer["topics"][pos - 1]
